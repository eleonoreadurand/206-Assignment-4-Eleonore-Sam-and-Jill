...
ab_scatter<- as.data.frame(lobster_ab) %>% 
  ...
  labs(x= "Year", y= "Lobster Count", caption = str_wrap(c("Figure 1. Lobster sizes at five research sites. Mean lobster size (mm) for Isla Vista, Naples, Mohawk, Arroyo Quemado, and Carpinteria (n = 1668 for each) in the Santa Barbara Channel. Error bars indicate +/- 1 standard deviation. Like letters indicate values that do not differ significantly (by one-way ANOVA with Tukey’s HSD; F(3.94) = 85.0, p= 0.009), with α = 0.05 for all post-hoc pairwise comparisons).")))+ # Figure 1 INSERT CAPTION HERE
...
pressure_table_pretty <- pressure_trend %>% 
  ...
  kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = FALSE) %>% 
  add_header_above(c("", "Table 2: Lobster Fishing Pressure at Sites, 2012-2017 "=9), escape= FALSE)
...
pressure_scatter<- as.data.frame(traps) %>% 
  ...
  scale_color_manual(labels= c("Arroyo Burro Reef", "Arroyo Hondo Reef", "Arroyo Hondo Reef to Arroyo Quemado Reef", "Arroyo Quemado Reef", "Carpinteria Reef", "Goleta Bay Reef", "Isla Vista Reef", "Mohawk Reef", "Naples Reef"), values= c("gold1", "plum3", "chocolate2", "cornflowerblue", "firebrick4", "darkolivegreen", "lightpink2", "darkslategrey")) +
...
aov_lob_graph <- ggplot(lobster_new, aes( x=SITE, y=mean_lobster_size)) +
  ...
  labs(x = "Site", y= "Average Carapace Length (mm)", caption = str_wrap(c("Figure 3: Lobster sizes at five research sites. Mean lobster size (mm) for Isla Vista, Naples, Mohawk, Arroyo Quemado, and Carpinteria (n = 1668 for each) in the Santa Barbara Channel. Error bars indicate +/- 1 standard deviation. Like letters indicate values that do not differ significantly (by one-way ANOVA with Tukey’s HSD; F(3.94) = 85.0, p= 0.009), with α = 0.05 for all post-hoc pairwise comparisons)."), width = 100)) +
  ...
  theme_classic()+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),text = element_text(family = "Times New Roman"), plot.caption=element_text(size=10, hjust=0, margin=margin(t=15)))
