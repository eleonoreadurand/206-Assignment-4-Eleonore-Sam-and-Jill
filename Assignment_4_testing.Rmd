...
ab_scatter<- as.data.frame(lobster_ab) %>% 
  ...
  labs(x= "Year", y= "Lobster Count", caption = str_wrap(c("Figure 1: California spiny lobster abundance by site in years 2012 to 2017.")))+ # Figure 1 INSERT CAPTION HERE
...
pressure_scatter<- as.data.frame(traps) %>% 
  ...
  theme_classic()+
  labs(x= "Year", y= "Number of traps", caption = str_wrap(c("Figure 2: Lobster fishing pressure by site in years 2012 to 2017.")))+ # Figure 2 INSERT CAPTION HERE
...
aov_lob_graph <- ggplot(lobster_new, aes( x=SITE, y=mean_lobster_size)) +
  ...
  labs(x = "Site", y= "Average Carapace Length (mm)", caption = str_wrap(c("Figure 3: Lobster sizes at five research sites. Mean lobster size (mm) for Isla Vista, Naples, Mohawk, Arroyo Quemado, and Carpinteria (n = 1668 for each) in the Santa Barbara Channel. Error bars indicate +/- 1 standard deviation. Like letters indicate values that do not differ significantly (by one-way ANOVA with Tukey’s HSD; F(4,1663) =3.42, p= 0.01), with α = 0.05 for all post-hoc pairwise comparisons)."), width = 100)) +
